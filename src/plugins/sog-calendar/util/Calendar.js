 ...
 *
 * @param {年份} year
 * @returns year年闰哪个月 1-12 , 没闰月传回 0
 */
 ...
 *
 * @param {日期} objDate
 * @returns 农历日期（农历年year 农历月month 农历日day 是否闰年isLeap yearCyl dayCyl monCyl）
 */
 ...
 *
 * @param {日期} dateVal
 * @returns Calendar对象，包含农历信息
 */
function initCalendar(dateVal) {
  const _date = dateVal ? dateVal : new Date();

  let GY = _date.getFullYear();
  let GM = _date.getMonth();
  let GD = _date.getDate();
  ...
  month = month.toString().padStart(2, "0"); // 小于10则补0
  ...
  calendar.weekday = weekday[_date.getDay()];
